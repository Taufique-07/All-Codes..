...
bool isLegend(const string& playerName)
{

    vector<string> legends = {"Ronaldo", "Messi", "Neuer", "Modric", "Kroos"};


    return find(legends.begin(), legends.end(), playerName) != legends.end();
}
...
int main()
{
    ...
    for (int i = 0; i < numTeams; ++i)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            ...
            /
            if (isLegend(teams[i].teamPlayers[j].name))
            {
                cout << "Status: Legend\n";
            }
            else
